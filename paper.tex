...
Baroni~\shortcite{baroni-dinu-kruszewski:2014:P14-1} conducted a set of
...
detection tasks Levy~\shortcite{TACL570} conducted a comprehensive set of
...
We take these results to use \texttt{word2vec} word embeddings in our
architecture because they are (1) often faster to learn, and (2) for
educational purposes, i.e.~we wanted to get some experience in using them.
\fixme[margin]{Line 70: "...often faster..." is there some evidence / citation for that?}
\fixme[margin]{Line 71: "...we wanted to get some experience..." -> nobody cares}

\section{Design}
\label{sec:design}
Our overall design takes inspiration from as far back as
Benello~\shortcite{Benello1989} who used a few preceding and one following word
in a feed-forward network with back-propagation for PoS tagging;
they achieved an accuracy of $0.95$ on the Brown Corpus.

Further inspiration comes from dos Santos~\shortcite{santos2014learning} who
used a deep neural network trained with character-level joint with word
embedding representation of words for PoS tagging. On the Wall Street Journal
(WSJ) portion of the Penn Treebank they achieve accuracy scores of $0.9732$. 

Our design comprises character-level 3-grams of word beginnings and ends joint
with word embedding representation of words in a long short-term memory (LSTM)
recurrent neural network (RNN)
architecture~\cite{Hochreiter:1997:LSM:1246443.1246450} for learning a mapping
of sequences of these representations to PoS tags.

\section{Implementation}
\label{sec:implementation}

We maintain the implementation in a source code repository at
\url{https://github.com/iiegn/bot_zen}.  The version tagged as {\tt 0.9}
comprises the version that was used to generate the results submitted to the
ST; 
...
For learning word embeddings we use \texttt{word2vec}; for building our neural
netowrk we use Keras, a minimalist, highly modular neural networks library,
written in Python and capable of running on top of either TensorFlow or
Theano~\cite{chollet2015}; in our case it runs on top of Theano, a Python
library that allows to define, optimize, and evaluate mathematical expressions
involving multi-dimensional arrays
efficiently~\cite{Theano2016}.
...
third person. See Bei{\ss}wenger~\shortcite{empirist2016} for more details.
...
\fixme[margin]{Line 187: as far as I know the trial data is part of the training data. Testing
on this will naturally yield good, you are training on your evaluation data.
What is the performance on the provided test set?}
...
negative sampling, few training epochs are suboptimal.}% https://groups.google.com/forum/#!topic/word2vec-toolkit/WUWad9fL0jU
) also with hierarchical softmax.
\fixme[margin]{Line 208: you should avoid terms like accidental. Is it even possible to use
negative sampling and hierarchical softmax? To my understanding these
approaches are different techniques that cannot be combined easily. By quickly
doublechecking the word2vec source code I think only HS is used if it is
activated, negative-sampling will be disregarded in this case. The authors
should check that thoroughly though.}
...
\fixme[margin]{Line 223: what are the default settings, i.e. they might change over time, it
is better to explicitly mention them}
...
2:~$0.82$,
3:~$0.88$.
\fixme[inline]{more verbose results.}
...
\fixme[margin]{When explaining numbers to the power of 10 write $9x10^8$ instead of $0.9*10^9$, also,
in running text it is advised to use "900 million" instead.}
...
\fxnote[inline]{Post EmpiriST experiment results / discussion and conclusion is still missing.}
